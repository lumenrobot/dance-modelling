...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_nid_cvst_corner_detection_process_check_sum_call( int
    nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_nid_cvst_corner_detection_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_nid_cvst_corner_detection_autoinheritance_info( int
    nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  ...
  if (strcmp(machineName, "nid_cvst_corner_detection") == 0) {
    const mxArray *newRhs[3] = { NULL, NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_nid_cvst_corner_detection_autoinheritance_info(nlhs,plhs,3,newRhs);
  }
...
{
  extern unsigned int sf_nid_cvst_corner_detection_third_party_uses_info( int
    nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  char machineName[128];
  if (nrhs < 4) {
  ...
  if (strcmp(machineName, "nid_cvst_corner_detection") == 0) {
    ...
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_nid_cvst_corner_detection_third_party_uses_info(nlhs,plhs,3,newRhs);
  }
...
{
  extern unsigned int
    sf_nid_cvst_corner_detection_get_eml_resolved_functions_info( int nlhs,
  ...
  if (strcmp(machineName, "nid_cvst_corner_detection") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_nid_cvst_corner_detection_get_eml_resolved_functions_info(nlhs,
      plhs,2,newRhs);
  }
...
extern unsigned int sf_nid_cvst_corner_detection_method_dispatcher(SimStruct *S,
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"nid_cvst_corner_detection")) {
    return(sf_nid_cvst_corner_detection_method_dispatcher(simstructPtr,
...
extern void nid_cvst_corner_detection_initializer(void);
extern void nid_cvst_corner_detection_register_exported_symbols(SimStruct* S);
extern void nid_cvst_corner_detection_debug_initialize(struct
  SfDebugInstanceStruct*);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  nid_cvst_corner_detection_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfGlobalMdlStartCallCounts == 1) {
    ...
    if (!simModeIsRTWGen) {
      nid_cvst_corner_detection_debug_initialize(sfGlobalDebugInstanceStruct);
    }

    nid_cvst_corner_detection_initializer();
  }
